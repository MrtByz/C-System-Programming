...
int main(int argc, char **argv){
    ...
    if(input_flag==0){
    	fprintf(stderr, "Usage : For argument (input file argument) not given\n");       /*Dosya acilamadiginda kullanıcıya hata mesajı verdirdim*/
		return -1;      
    }
    if(argc!=3){
    	fprintf(stderr, "Usage : only input file argument should be given \n");       /*Dosya acilamadiginda kullanıcıya hata mesajı verdirdim*/
		return -1;      
    }
    ...
    sem3 = mmap(NULL, sizeof(sem3), PROT_READ |PROT_WRITE,MAP_SHARED|MAP_ANONYMOUS,-1, 0);
    semp = mmap(NULL, sizeof(semp), PROT_READ |PROT_WRITE,MAP_SHARED|MAP_ANONYMOUS,-1, 0);
